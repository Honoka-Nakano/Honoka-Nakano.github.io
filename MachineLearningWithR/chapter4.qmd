...
`np.meshgrid(w0, w1)`は，2次元平面でのすべての座標を出力するコードである．例えば，$($ `ww0[1, 1]` $,$ ` ww1[1, 1]` $)$は$(\mathrm{w}_0, \mathrm{w}_1) = (-2, -2)$を，$($ `ww0[1, 2]` $,$ `ww1[1, 2]` $)$ は$(\mathrm{w}_0, \mathrm{w}_1) = (-2, -1.75)$を表している．`ww0`と`ww1`の対応する要素が各座標を表しているのである．
...
`expand_grid()`を用いて座標を作ったとしても，グラフの描画に用いる`{ggplot2}`ではとある理由(後述)で扱えないため，無理に使う必要はない．
...
```{r}
# R
df <- array(NA, dim = c(w0_n * w1_n, 5)) |>
  data.frame() |>
  rename(ww0   = 'X1', ww1   = 'X2', f_num = 'X3', dw0   = 'X4', dw1   = 'X5')
n <- 1                                      #  便宜上のインデックス．
for (i in 1:w0_n) {
  for (j in 1:w1_n) {
